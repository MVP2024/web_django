...
def get_products_by_category_cached(category_pk: int):
    ...
    products = None  # Инициализируем products

    if settings.CACHE_ENABLED:  # Проверяем, включено ли кеширование
        # Используем кеш по умолчанию для чтения, так как он может содержать данные
        products = caches['default'].get(cache_key)  # Обращаемся у кешу по имени

    if products is None:
        # Если данные не в кеше или кеширование отключено, получаем их из БД
        category = get_object_or_404(Category, pk=category_pk)
        products = list(
            Product.objects.filter(category=category, is_published=True).select_related('category').order_by('name'))

        if settings.CACHE_ENABLED:  # Только кешируем данные, если кеширование включено
